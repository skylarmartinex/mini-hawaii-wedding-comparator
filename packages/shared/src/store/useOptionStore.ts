...
export const useOptionStore = create<OptionStore>((set, get) => ({
  ...
  hydrate: async () => {
    ...
    if (adapter.migrateIfNeeded) await adapter.migrateIfNeeded();
    let options = await adapter.loadOptions();
    // Auto-load seed data on first visit
    if (options.length === 0) {
      await adapter.saveAllOptions(SEED_OPTIONS);
  ...
  addOption: async (data) => {
    ...
    await adapter.saveOption(option);
  ...
  updateOption: async (id, patch) => {
    ...
    await adapter.saveOption(updated);
    set((s) => ({
  ...
  importOptions: async (options) => {
    ...
    await adapter.saveAllOptions(validated);
